...
Correios.prototype.calcPreco = function(args, callback) {
  'use strict';
  this.validArg(args,function(err,result){
    if(err) {
      throw err;
    }else{
      args = result;     
...
Correios.prototype.validArg = function(args,callback){
    //decimal is number
    var fields = {
      nCdServico :'string',
      sCepOrigem :'string',
    ...
    for(key in fields){
      ...
      if(args[key] != undefined){
          ...
          if(key == 'nCdFormato' && args['nCdFormato'] == 3){
            if(args['nVlPeso'] > 1){
              erros.push({Erro: true,MsgErro:'nVlPeso cannot be bigger then 1Kg when "envelope" is choosed'});
            }
          ...
          if(typeof args[key] != fields[key]){ // the type of the field is right?
            erros.push({Erro: true,MsgErro:"Field "+key+" MUST be "+fields[key]});
          } 
      }else{
        erros.push({Erro: true,MsgErro:"You MUST set "+key+"!"});
      }
    }
