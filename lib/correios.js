  ...
  request.post({
    ...
    if (err) {
      ...
      if (err.code == 'ENOTFOUND') {
        return _this.emit('error', 'Sem conexão com a internet');
      }
    }
    ...
    if (resp.statusCode === 200) {
      ...
      //If result
      if ($('.caixacampoazul')) {
        var keys = $('.resposta').map(function (i, el) {
          var text = $(this).text().toLowerCase();
          return text.replace(':', '').replace('/', '-').replace(/\s/g, '');
        }).get();

        var vals = $('.respostadestaque').map(function (i, el) {
        ...
        return _this.emit('error', 'CEP não encontrado');
      }
